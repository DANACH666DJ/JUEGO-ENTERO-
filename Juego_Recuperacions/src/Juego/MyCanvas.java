...
public class MyCanvas extends Canvas {
	...
	public void BajarMenu(){
		iposicion_menu++;
		if(iposicion_menu>3)iposicion_menu = 0;
		repaint();
	}
	public void SubirMenu(){
		iposicion_menu--;
		if(iposicion_menu<0)iposicion_menu = 3;
		repaint();
	}
	...
	public void PaintMenu(Graphics2D p) {
		...
		if(iposicion_menu==0)p.setColor(Color.RED);
		p.drawString("START", 350, 350);
		p.setColor(Color.black);
		
		if(iposicion_menu==1)p.setColor(Color.RED);
		p.drawString("SETTINGS", 350, 370);
		p.setColor(Color.black);
		
		if(iposicion_menu==2)p.setColor(Color.RED);
		p.drawString("NO SIRVE", 350, 390);
		p.setColor(Color.black);
		
		if(iposicion_menu==3)p.setColor(Color.RED);
